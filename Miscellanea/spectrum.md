...
なお，スペクトル（スペクトラム）という用語は上記の可換環のスペクトルや線形作用素のスペクトル以外にも様々な場面で用いられるが（分光スペクトル，スペクトル系列，$\Omega$スペクトラム，etc...），これらの用語についても最後に少しだけ触れたい．
...
## 参考文献
* \biblabel{Hiai-Yanagi}{[HY]} 日合-柳, ヒルベルト空間と線型作用素, 牧野書店.
* \biblabel{Ikunishi-Nakagami}{[IN]} 行西-中神, 作用素環入門I, 岩波書店.
* \biblabel{Ueno}{[U]} 上野, 代数幾何, 岩波書店.
